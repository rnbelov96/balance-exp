...
export default (form: HTMLFormElement) => {
  ...
  const cityErrorLabelEl = form.querySelector('[data-error-label="city"]') as HTMLDivElement | null;
  ...
  if (phoneInputEl.value !== '' && !isMobilePhone(`${phoneInputEl.value.replace(/\(|\)|-|_/g, '')}`, 'ru-RU')) {
    phoneInputEl.classList.add('input-error');
